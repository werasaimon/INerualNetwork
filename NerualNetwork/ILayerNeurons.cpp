...
void ILayerNeurons::InitRandomWeight()
{
    ...
    {
        errorsBias[outp] = (unif(rng) - 0.5) * 3 +(unif(rng) - 0.5);
    }
    ...
    for(int inp =0; inp < in_count+1; inp++)
    {
        for(int outp =0; outp < out_count; outp++)
        {
            getMatrix(inp,outp) =  (unif(rng) - 0.5) * 3 +(unif(rng) - 0.5);
        }
    }
}
...
std::string ILayerNeurons::serialize() const
{
    ...
    unsigned int nbrOfDoublesWeightMatrix = (in_count+1) * out_count; // + in_count;
    ...
    unsigned int nbrOfDoublesBias = out_count;
    float* biasBuf = new float[ nbrOfDoublesBias ];
    for( unsigned int m = 0; m < nbrOfDoublesBias; m++ )
    {
...
ILayerNeurons *ILayerNeurons::deserialize(const std::string &buffer)
{
    ...
    for( unsigned int m = 0; m < nbrOfNeurons+1; m++ )
    {
        for( unsigned int n = 0; n < nbrOfOutputs; n++ )
        {
    ...
    for( unsigned int m = 0; m < nbrOfOutputs; m++ )
    {
